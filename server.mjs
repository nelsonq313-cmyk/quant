...
app.get('/api/status',(_req,res)=>res.json({server:{ok:true,now:new Date().toISOString(),cacheEntries:cache.size},marketData:{configured:Boolean(process.env.MARKETDATA_TOKEN),mode:'entitlement dependent; QNT labels provider market data as delayed unless verified otherwise',...API_STATE.marketData},openai:{configured:Boolean(process.env.OPENAI_API_KEY),models:{fast:process.env.OPENAI_FAST_MODEL||defaultModels.fast,research:process.env.OPENAI_RESEARCH_MODEL||defaultModels.research,deep:process.env.OPENAI_MODEL||defaultModels.deep},tools:['on-demand code_interpreter','optional web_search'],...API_STATE.openai}}));
