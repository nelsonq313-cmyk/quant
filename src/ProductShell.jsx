...
import {
  Activity, BarChart3, BookOpen, Boxes, ChevronRight, Command, Database, FileCode2, Folder, FolderKanban,
...
function LeftRail({section,setSection}){
  const items=[['Home',LayoutDashboard],['Desk',BarChart3],['Analytics',Boxes],['Projects',LayoutGrid],['Copilot',MessageSquareText],['Market',Gauge],['Library',BookOpen],['API',Server]];
...
function parseAnalyticsQuery(text){
  ...
  const symbol=parts.slice(1).find(x=>/^[A-Z0-9.\-]{1,12}$/.test(x));
  return {fn,symbol};
}
...
function CommandPalette({open,onClose,setSection}){
  ...
  const run=item=>{const [code,,,type,target]=item;if(type==='shell')setSection(target);else if(type==='research'){setSection('Copilot');emitResearch(target)}else if(type==='desk'){runDesk({fn:target});return}else if(type==='analytics'){runAnalytics({fn:target});return}else{setSection('Market');try{sessionStorage.setItem('qnt.market.command',JSON.stringify({type,target}))}catch{}window.setTimeout(()=>emitMarket({type,target}),0)}saveRecent(code);onClose()};
  ...
  return <div className="qpsCommandBackdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><div className="qpsCommand"><div className="qpsCommandInput"><Command size={15}/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')onClose();if(e.key==='Enter'){e.preventDefault();submit()}}} placeholder="DES QQQ · CHAIN AAPL · RVIV QQQ · MC · ask QNT…"/><kbd>ESC</kbd></div><div className="qpsCommandList">{q&&<button className="qpsAskCommand" onClick={()=>askQnt(query)}><kbd>ASK</kbd><div><b>Ask QNT Copilot</b><span>{query}</span></div><ChevronRight size={13}/></button>}{visible.map(item=><button key={item[0]} onClick={()=>run(item)}><kbd>{item[0]}</kbd><div><b>{item[1]}</b><span>{item[2]}</span></div><ChevronRight size={13}/></button>)}</div><div className="qpsCommandFoot">Examples: DES QQQ · CHAIN AAPL · SURF QQQ · SKEW AAPL · TERM AAPL · RVIV QQQ · GREEKS AAPL · OI AAPL</div></div></div>;
}
