...
const COMMANDS=[
  ['LAB','Research Lab','AI + Python strategy experiments, statistical tests, charts and 3D artifacts'],
...
const researchMap={LAB:'Research Lab',WORK:'Research Lab',VALIDATE:'Validation',VERDICT:'Verdict',REGIME:'Regimes',MC:'Risk & Monte Carlo',PROP:'Prop Firm',VOL:'Volatility'};
...
function CommandPalette({open,onClose,run}){
  ...
  if(!open)return null;
  return <div className="q4CommandBackdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><div className="q4Command"><div className="q4CommandSearch"><Search size={16}/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')onClose();if(e.key==='Enter'){e.preventDefault();run(q||visible[0]?.[0])}}} placeholder="Research Lab, validation, Monte Carlo, journal…"/><kbd>ESC</kbd></div><div className="q4CommandRows">{visible.map(x=><button key={x[0]} onClick={()=>run(x[0])}><kbd>{x[0]}</kbd><div><b>{x[1]}</b><span>{x[2]}</span></div><ChevronRight size={14}/></button>)}</div><div className="q4CommandHint">LAB · VALIDATE · VERDICT · MC · JOURNAL</div></div></div>;
}

export default function QntRedesign(){
  const[section,setSection]=useState('Research'),[commandOpen,setCommandOpen]=useState(false);
  const go=(next,tab)=>{if(section==='Research'&&next!=='Research')emit('qnt:navigate',{type:'research',tab:'Research Lab'});setSection(next);if(next==='Research'&&tab)setTimeout(()=>emit('qnt:navigate',{type:'research',tab}),0)};
  ...
  useEffect(()=>{const key=e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();setCommandOpen(true)}if(e.key==='Escape')setCommandOpen(false)},route=e=>{const d=e.detail||{};if(d.section==='Research')go('Research',d.tab||'Research Lab')};window.addEventListener('keydown',key);window.addEventListener('qnt:route',route);return()=>{window.removeEventListener('keydown',key);window.removeEventListener('qnt:route',route)}},[section]);
