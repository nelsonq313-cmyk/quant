...
function buildBands(paths,steps,start){
  ...
  for(let step=0;step<=steps;step++){
    ...
    for(let i=0;i<paths.length;i++){const value=paths[i][step];values[i]=value;if(value<start)below+=1}
    values.sort((a,b)=>a-b);
...
function validateConfig(cfg){
  ...
  const riskPct=cfg.start>0?cfg.risk/cfg.start*100:0,budget=Math.max(0,cfg.start-cfg.floor),budgetPct=budget?cfg.risk/budget*100:0;
...
function simulate(cfg,rawMode,seed){
  ...
  for(let base=0;base<cfg.simulations;base+=blockSize){
    ...
    for(let s=base;s<end;s++){
      ...
      for(let step=1;step<=cfg.steps;step++){
        ...
        if(equity<=cfg.floor){
          if(firstBoundary==null)firstBoundary='ruin';
          ruinStep=step;floorHits+=1;localRuin+=1;timesToFloor.push(step);
          break;
        }
        if(firstBoundary==null&&equity>=cfg.target){
          firstBoundary='target';targetStep=step;targetFirst+=1;localTarget+=1;timesToTarget.push(step);
        }
      }

      const endStep=ruinStep??cfg.steps;
      if(ruinStep!=null){for(let step=ruinStep+1;step<=cfg.steps;step++)paths[s][step]=equity}
      ...
      meta[s]={status:ruinStep!=null?'ruined':targetStep!=null?'target':'survived',endStep,ruinStep,targetStep,reason:ruinStep!=null?`Ruin floor crossed on trade ${ruinStep}`:targetStep!=null?`Target reached before ruin on trade ${targetStep}`:'Survived full horizon',finalEquity:equity,sampledWinRate:probs.win*100};
    }
  ...
  return {paths,tradeRs,meta,bands,terminals,pnl,returns,maxDds,losing,underwater,timesToFloor,timesToTarget,ruinPct,survivePct:100-ruinPct,profitPct:profitAndSurvive/cfg.simulations*100,targetPct:targetFirst/cfg.simulations*100,neitherPct:neither/cfg.simulations*100,meanPnl:mean(pnl),medianPnl:median(pnl),p5Pnl,p95Pnl:quantile(pnl,.95),medianReturn:median(returns),p95Dd,medianDd:median(maxDds),p25Dd:quantile(maxDds,.25),p75Dd:quantile(maxDds,.75),p95Losing:quantile(losing,.95),p95Underwater:quantile(underwater,.95),medianRecovery:median(recoveries),riskScore:riskIndex,tradeSharpe:ratios.tradeSharpe,sortino:ratios.sortino,ruinCI:[quantile(scenarioRuin,.05),quantile(scenarioRuin,.95)],profitCI:[quantile(scenarioProfit,.05),quantile(scenarioProfit,.95)],targetCI:[quantile(scenarioTarget,.05),quantile(scenarioTarget,.95)],modelWinCI:[quantile(sampledWinRates,.05),quantile(sampledWinRates,.95)]};
}
...
function CanvasPaths({result,cfg,cursorStep,setCursorStep,onSelectPath}){
  ...
  useEffect(()=>{
    ...
    const lo=Math.min(cfg.floor,...band.map(x=>x.p05)),hi=Math.max(cfg.target,...band.map(x=>x.p95)),yPad=Math.max(300,(hi-lo)*.08),minY=lo-yPad,maxY=hi+yPad;
    const xOf=s=>pad.l+(s/cfg.steps)*(w-pad.l-pad.r),yOf=v=>pad.t+(maxY-v)/(maxY-minY)*(h-pad.t-pad.b);

    ctx.fillStyle='#050607';ctx.fillRect(0,0,w,h);ctx.font='8px ui-monospace, monospace';ctx.textAlign='right';ctx.textBaseline='middle';
    for(let i=0;i<5;i++){const value=minY+(maxY-minY)*(i/4),y=yOf(value);ctx.strokeStyle='rgba(109,116,126,.16)';ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();ctx.fillStyle='#596069';ctx.fillText(money1(value),pad.l-7,y)}
    ctx.textAlign='center';ctx.textBaseline='top';for(let step=0;step<=cfg.steps;step+=Math.max(1,Math.round(cfg.steps/5))){ctx.fillStyle='#555c64';ctx.fillText(String(step),xOf(step),h-pad.b+6)}
    for(const [value,color,label] of [[cfg.target,'#2ad4a1','Target'],[cfg.floor,'#ef6670','Ruin floor']]){const y=yOf(value);ctx.strokeStyle=color;ctx.globalAlpha=.48;ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=1;ctx.textAlign='right';ctx.textBaseline='bottom';ctx.fillStyle=color;ctx.fillText(label,w-pad.r-4,y-3)}

    const maxVisible=700,stride=Math.max(1,Math.ceil(result.paths.length/maxVisible)),drawIndices=[],palette=['rgba(138,114,239,.18)','rgba(55,154,220,.14)','rgba(43,201,155,.13)','rgba(213,161,71,.11)'];
    for(let p=0,draw=0;p<result.paths.length;p+=stride,draw++){
      drawIndices.push(p);const path=result.paths[p],meta=result.meta[p],end=meta.endStep;ctx.strokeStyle=meta.status==='ruined'?'rgba(239,102,112,.22)':palette[draw%palette.length];ctx.lineWidth=.55;ctx.beginPath();
      for(let step=0;step<=end;step++){const x=xOf(step),y=yOf(path[step]);if(step===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.stroke();
    }
    ctx.strokeStyle='rgba(225,220,255,.88)';ctx.lineWidth=1.3;ctx.beginPath();band.forEach((b,i)=>{const x=xOf(b.step),y=yOf(b.p50);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke();ctx.strokeStyle='rgba(138,114,239,.28)';ctx.lineWidth=.8;for(const key of ['p10','p90']){ctx.beginPath();band.forEach((b,i)=>{const x=xOf(b.step),y=yOf(b[key]);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke()}
  ...
  useEffect(()=>{
    ...
    if(hover.path!=null){const path=result.paths[hover.path],meta=result.meta[hover.path],end=meta.endStep;ctx.strokeStyle=meta.status==='ruined'?'#ef6670':'#f2efff';ctx.lineWidth=2;ctx.beginPath();for(let s=0;s<=end;s++){const x=scale.xOf(s),y=scale.yOf(path[s]);if(s===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.stroke();const dotStep=Math.min(step,end);ctx.fillStyle=ctx.strokeStyle;ctx.beginPath();ctx.arc(scale.xOf(dotStep),scale.yOf(path[dotStep]),3,0,Math.PI*2);ctx.fill()}
  },[hover,result,cursorStep]);

  const move=e=>{
    const scale=scaleRef.current;if(!scale)return;
    const rect=e.currentTarget.getBoundingClientRect(),x=clamp(e.clientX-rect.left,scale.pad.l,rect.width-scale.pad.r),y=e.clientY-rect.top,step=clamp(Math.round(((x-scale.pad.l)/(rect.width-scale.pad.l-scale.pad.r))*cfg.steps),0,cfg.steps);
    let nearest=null,best=Infinity;for(const p of scale.drawIndices){const end=result.meta[p].endStep;if(step>end)continue;const dy=Math.abs(scale.yOf(result.paths[p][step])-y);if(dy<best){best=dy;nearest=p}}
    setCursorStep(step);setHover({path:best<=7?nearest:null,x:e.clientX-rect.left,y,step});
  };

  const b=result.bands[hover.step]||result.bands.at(-1),path=hover.path!=null?result.paths[hover.path]:null,meta=hover.path!=null?result.meta[hover.path]:null,boxLeft=clamp(hover.x+14,8,(scaleRef.current?.w||400)-218),boxTop=clamp(hover.y-58,8,(scaleRef.current?.h||300)-92);
  return <div className="qmcCanvasWrap"><canvas ref={baseRef} className="qmcCanvas qmcBaseCanvas"/><canvas ref={overlayRef} className="qmcCanvas qmcOverlayCanvas" onPointerMove={move} onPointerLeave={()=>setHover(h=>({...h,path:null}))} onClick={()=>hover.path!=null&&onSelectPath(hover.path)}/><div className={`qmcCursorBox ${hover.path!=null?'pathHit':''}`} style={{left:boxLeft,top:boxTop,right:'auto'}}>{hover.path!=null?<><span>PATH #{hover.path+1} · TRADE {Math.min(hover.step,meta.endStep)}</span><b>{money1(path[Math.min(hover.step,meta.endStep)])}</b><small>{meta.status.toUpperCase()} · {meta.reason}</small><small>click for path log</small></>:<><span>TRADE {hover.step}</span><b>{money1(b.p50)}</b><small>p10 {money1(b.p10)} · p90 {money1(b.p90)}</small><small>{b.belowPct.toFixed(0)}% below start · {(100-b.belowPct).toFixed(0)}% above</small></>}</div></div>;
}

function PathDrawer({index,result,cfg,onClose}){
  if(index==null)return null;
  const path=result.paths[index],rs=result.tradeRs[index],meta=result.meta[index],rows=[];for(let step=1;step<=meta.endStep;step++)rows.push({step,r:rs[step],delta:path[step]-path[step-1],equity:path[step]});
  return <aside className="qmcPathDrawer"><div className="qmcDrawerHead"><div><span className={`qmcStatusPill ${meta.status==='ruined'?'bad':meta.status==='target'?'good':'warn'}`}>{meta.status.toUpperCase()}</span><b>Path #{index+1} · {meta.endStep} trades</b><small>{meta.reason} · Final {money(meta.finalEquity)} · sampled WR {meta.sampledWinRate.toFixed(1)}%</small></div><button onClick={onClose}><X size={15}/></button></div><div className="qmcDrawerResult"><b>{meta.status==='ruined'?'Account stopped at ruin':meta.status==='target'?'Target was reached':'Full horizon survived'}</b><span>Once a path hits ruin, no later trades are generated for that account.</span></div><div className="qmcTradeLogHead"><span>TRADE</span><span>R*</span><span>P&L</span><span>EQUITY</span></div><div className="qmcTradeLog">{rows.map(row=><div className="qmcTradeRow qmcTradeRow4" key={row.step}><span>{row.step}</span><span className={row.r>=0?'good':'bad'}>{row.r>=0?'+':''}{row.r.toFixed(2)}</span><span className={row.delta>=0?'good':'bad'}>{row.delta>=0?'+':''}{money(row.delta)}</span><span>{money(row.equity)}</span></div>)}</div></aside>;
}

function Validation({validation}){if(!validation.errors.length&&!validation.warnings.length)return null;return <div className={`qmcValidation ${validation.errors.length?'error':'warn'}`}><AlertTriangle size={13}/><div>{validation.errors.map(x=><b key={x}>{x}</b>)}{validation.warnings.map(x=><span key={x}>{x}</span>)}</div></div>}

function Settings({draft,setDraft,rawDraft,setRawDraft}){
  const set=(key,value)=>setDraft(cur=>({...cur,[key]:Number(value)}));
  return <div className="qmcSettings qmcRiskV2Settings"><label>Simulations<input value={draft.simulations} onChange={e=>set('simulations',e.target.value)}/></label><label>Horizon<input value={draft.steps} onChange={e=>set('steps',e.target.value)}/></label><label>Start<input value={draft.start} onChange={e=>set('start',e.target.value)}/></label><label>Risk / trade<input value={draft.risk} onChange={e=>set('risk',e.target.value)}/></label><label>Ruin floor<input value={draft.floor} onChange={e=>set('floor',e.target.value)}/></label><label>Profit target<input value={draft.target} onChange={e=>set('target',e.target.value)}/></label><label>Calibration<select value={rawDraft?'raw':'conservative'} onChange={e=>setRawDraft(e.target.value==='raw')}><option value="conservative">Conservative + uncertainty</option><option value="raw">Raw empirical + uncertainty</option></select></label></div>;
}

function CrossSection({result,cfg,crossStep,setCrossStep}){
  const values=useMemo(()=>result.paths.map(p=>(p[crossStep]-cfg.start)/cfg.start*100),[result,cfg.start,crossStep]),data=useMemo(()=>histogram(values,30),[values]),med=median(values),under=values.filter(x=>x<0).length/(values.length||1)*100;
...
function Sensitivity({cfg,rawMode,seed}){
  ...
  const rows=useMemo(()=>{
    const wins=rawMode?rawWins:conservativeWins,winBase=personalModel.winProbability,be=personalModel.breakevenProbability,winScenarios=[-.15,-.10,-.05,0,.05,.10,.15].map(x=>clamp(winBase+x,.05,.9));
    return winScenarios.map((winRate,ri)=>({winRate,cells:riskMultipliers.map((mult,ci)=>{
      ...
      for(let s=0;s<sims;s++){let equity=cfg.start;for(let step=1;step<=cfg.steps;step++){equity+=drawR(random,wins,{win:winRate,loss:lossRate,breakeven:be})*cfg.risk*mult;if(equity<=cfg.floor){ruined+=1;break}}}
      return {mult,ruin:ruined/sims*100};
    })}));
  },[cfg,rawMode,seed]);
  return <section className="qmcSensitivity"><div className="qmcSensitivityHead"><div><b>SENSITIVITY · WIN RATE × RISK</b><span>Fast scenario grid using the same payoff library. Directional stress test, not the posterior estimate above.</span></div></div><div className="qmcHeatGrid" style={{gridTemplateColumns:`110px repeat(${riskMultipliers.length},1fr)`}}><div className="head">Scenario WR</div>{riskMultipliers.map(m=><div className="head" key={m}>{m.toFixed(2)}× risk</div>)}{rows.flatMap(row=>[<div className="rowHead" key={`h-${row.winRate}`}>{(row.winRate*100).toFixed(1)}% WR</div>,...row.cells.map(cell=>{const alpha=clamp(cell.ruin/35,.08,.9);return <div className="heat" key={`${row.winRate}-${cell.mult}`} style={{background:`rgba(239,102,112,${alpha})`}}><b>{cell.ruin.toFixed(1)}%</b><span>P(ruin)</span></div>})])}</div></section>;
}
...
export default function RiskMonteCarloV2(){
  ...
  const validation=useMemo(()=>validateConfig(draft),[draft]),result=useMemo(()=>simulate(active,rawMode,seed),[active,rawMode,seed]);
  const run=(newSeed=false)=>{const safe={...draft,simulations:clamp(Math.round(Number(draft.simulations)||4000),100,10000),steps:clamp(Math.round(Number(draft.steps)||109),5,500)},check=validateConfig(safe);if(check.errors.length)return;setActive(safe);setDraft(safe);setRawMode(rawDraft);setCrossStep(v=>clamp(v,1,safe.steps));setCursorStep(safe.steps);setSelectedPath(null);if(newSeed)setSeed(s=>(s+104729)>>>0)};
  const terminalData=histogram(result.pnl,28),ddData=histogram(result.maxDds,24),underData=histogram(result.underwater,24),tabs=['Simulation','Sensitivity','Outcome distributions','Tail risk','Model audit'];
