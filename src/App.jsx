...
import{Activity,BarChart3,Braces,Database,Settings2,Sigma,TestTube2,Waves}from'lucide-react';
...
const tabs=[['Workspace',Braces],['Validation',TestTube2],['Prop Firm',Sigma],['Verdict',Activity],['Regimes',BarChart3],['Risk & Monte Carlo',BarChart3],['Volatility',Waves],['Data',Database]];
...
export default function App(){
  ...
  return <div className="appShell qpCore"><header className="topNav qpResearchNav"><nav>{tabs.map(([name,Icon])=><button key={name} className={tab===name?'active':''} onClick={()=>select(name)}><Icon size={12}/>{name}</button>)}</nav><div className="rightNav"><span className="statusDot"/> PERSONAL MODEL</div></header><main>{tab==='Workspace'&&<ResearchStudio/>}{tab==='Validation'&&<StrategyValidationLab/>}{tab==='Verdict'&&<ResearchVerdict/>}{tab==='Regimes'&&<RegimeLab/>}{tab==='Volatility'&&<VolatilityLab/>}{tab==='Data'&&<DataTerminal/>}{(tab==='Prop Firm'||tab==='Risk & Monte Carlo')&&<div className="qpLoadingTool"><Settings2 size={15}/><span>Opening simulation workspace…</span></div>}</main></div>;
}
