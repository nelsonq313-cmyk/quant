...
const sampleR = [
  ...
  -1,1.7,2.6,-1,4.2,-1,5.1,2.3,-1,-1,6.8,1.4,-1,3.5,2.9,-1,9.1,-1,1.6,4.4,
  -1,3.2,2.2,-1,5.6,-1,7.9,1.8,-1,3.7,-1,2.5,4.9,-1,6.1,-1,-1,3.3,2.7,-1,
  8.6,1.2,-1,4.1,-1,5.4,2.0,-1,3.8,-1,7.1,2.4,-1,-1,6.5,1.5,-1,4.6,2.8,-1,
  5.9,-1,3.0,7.7,-1,1.9,-1,4.3,2.6,-1,6.9,-1,3.4,2.1,-1,5.2,-1,8.0,1.6,-1,
];
...
function runMonteCarlo({ returns, simulations, steps, startingBalance, riskPerTrade, target, floor }) {
  ...
  return <div className="terminalGrid">
    <aside className="workspacePane panel">
      ...
      <div className="terminalFacts">
        <span>Rows</span><b>{returns.length}</b><span>Source</span><b>Journal / CSV</b>
      </div>
    </aside>
    ...
    <section className="resultsPane panel">
      ...
      <p className="finding">The sample shows positive expectancy with a favorable payoff profile. Volatility is elevated, so the edge is strongest when risk is kept fixed instead of scaled aggressively.</p>
...
function MonteCarloView({returns,propMode=false}){
  ...
  const result=useMemo(()=>runMonteCarlo({...cfg,returns}),[cfg,returns,seed]);
