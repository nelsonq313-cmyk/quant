...
function pearson(a,b){if(a.length!==b.length||a.length<3)return null;const ma=a.reduce((s,v)=>s+v,0)/a.length,mb=b.reduce((s,v)=>s+v,0)/b.length;let num=0,da=0,db=0;for(let i=0;i<a.length;i++){const x=a[i]-ma,y=b[i]-mb;num+=x*y;da+=x*x;db+=y*y}const den=Math.sqrt(da*db);return den?num/den:null}

export default function MarketTerminal(){
  const [quotes,setQuotes]=useState([]),[symbol,setSymbol]=useState('QQQ'),[query,setQuery]=useState('QQQ'),[compareSymbol,setCompareSymbol]=useState('SPY');
  const [candles,setCandles]=useState([]),[compareCandles,setCompareCandles]=useState([]),[health,setHealth]=useState(null),[loading,setLoading]=useState(false),[compareLoading,setCompareLoading]=useState(false),[error,setError]=useState(''),[coverageNotice,setCoverageNotice]=useState('');
  ...
  const fetchCandles=async clean=>{const r=await fetch(`/api/market/candles?symbol=${encodeURIComponent(clean)}&resolution=D&countback=90`),d=await r.json();if(!r.ok)throw new Error(d.error||'Candle request failed');return(d.candles||[]).map(x=>({...x,label:fmtTime(x.time)}))};
  const loadCompare=async next=>{const clean=String(next||'SPY').trim().toUpperCase();if(!clean||UNSUPPORTED_FUTURES.has(clean))return;setCompareLoading(true);try{setCompareCandles(await fetchCandles(clean));setCompareSymbol(clean)}catch(e){setError(e.message)}finally{setCompareLoading(false)}};
  const loadSymbol=async(next=symbol)=>{
    ...
    try{setCandles(await fetchCandles(clean));setSymbol(clean);setQuery(clean);setNotes(localStorage.getItem(`qnt.market.notes.${clean}`)||'');setError('');if(clean===compareSymbol){const fallback=clean==='QQQ'?'SPY':'QQQ';loadCompare(fallback)}}catch(e){setError(e.message)}finally{setLoading(false)}
  };
  ...
  useEffect(()=>{
    loadQuotes();loadHealth();loadSymbol('QQQ');loadCompare('SPY');
  ...
  const summary=useMemo(()=>{
    ...
    const last=candles.at(-1),prev=candles.at(-2),ret=prev?(last.close-prev.close)/prev.close*100:null,hi=Math.max(...candles.slice(-20).map(c=>c.high??c.close)),lo=Math.min(...candles.slice(-20).map(c=>c.low??c.close));
  ...
  const correlation=useMemo(()=>{
    const compareMap=new Map(compareCandles.map(c=>[c.time,c.close])),pairs=candles.filter(c=>compareMap.has(c.time)).map(c=>({a:c.close,b:compareMap.get(c.time)})).sort((x,y)=>0);if(pairs.length<4)return {value:null,n:0};
    const a=[],b=[];for(let i=1;i<pairs.length;i++){const pa=pairs[i-1],p=pairs[i];if(pa.a>0&&pa.b>0){a.push((p.a-pa.a)/pa.a);b.push((p.b-pa.b)/pa.b)}}return {value:pearson(a,b),n:a.length};
  ...
  return <div className="qtMarket">
    ...
    <div className="qtMarketGrid">
      ...
      <section className="qtCorrelation panel"><div className="qtPanelHead"><div><b>RETURN CORRELATION</b><span>aligned daily close-to-close returns</span></div><select value={compareSymbol} onChange={e=>loadCompare(e.target.value)}>{DEFAULT_SYMBOLS.filter(s=>s!==symbol).map(s=><option value={s} key={s}>{s}</option>)}</select></div><div className="qtCorrBody"><div><span>PAIR</span><b>{symbol} × {compareSymbol}</b></div><div><span>PEARSON r</span><b>{compareLoading?'…':correlation.value==null?'—':correlation.value.toFixed(3)}</b></div><div><span>ALIGNED RETURNS</span><b>{correlation.n}</b></div><p>Research statistic only. Direct NQ/ES/VIX/DXY correlations remain unavailable until those verified feeds are connected.</p></div></section>
